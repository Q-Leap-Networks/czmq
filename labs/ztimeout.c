...
struct _ztimeout_node_t {
    zdlist_t *node;                     // handle from zdlist
...
struct _ztimeout_t {
    zdlist_t *bucket [NUM_BUCKETS];      //  Buckets holding timeouts
...
ztimeout_new (ztimeout_trigger_fn trigger_fn)
{
    ...
    if (self) {
        ...
        for (int index = 0; index < NUM_BUCKETS; ++index) {
            self->bucket [index] = zdlist_insert_before (self->bucket [0], NULL);
        ...
        zdlist_set_destructor (self->bucket [0],
...
ztimeout_first (ztimeout_t *self)
{
    ...
    for (int index = 0; index < NUM_BUCKETS; ++index) {
        ...
        if (self->next_time [index] <= now) {
            ...
            zdlist_t *iterator = zdlist_first (self->bucket [index], NULL);
            while (iterator) {
                zdlist_t *next = zdlist_next (iterator, NULL);
                ztimeout_node_t *node =
                    (ztimeout_node_t *) zdlist_item (iterator);
                ...
                if (slot < 0) {
                    ...
                    zdlist_move_after (iterator, self->bucket [slot]);
        ...
        if (self->next_time [index] < timeout) {
            ...
            zdlist_t *iterator = zdlist_next (self->bucket [index], NULL);
            if (iterator) {
                // bucket isn't empty, not a false alarm
                if (zdlist_next (iterator, NULL))
...
ztimeout_add (ztimeout_t *self, void *item, int64_t time) {
    ...
    node->node = zdlist_insert_after (self->bucket [slot], node);
...
ztimeout_restart (ztimeout_t *self, ztimeout_node_t *node, int64_t time)
{
    ...
    assert (node);
    uint slot = s_calc_bucket (time);
    zdlist_move_after (node->node, self->bucket [slot]);
    node->time = zclock_mono () + time;
    node->current_slot = slot;
}
...
ztimeout_set_destructor (ztimeout_t *self, czmq_destructor destructor)
{
    ...
    zdlist_t *iterator = zdlist_first (self->bucket [0], self->bucket [0]);
    while (iterator) {
        ztimeout_node_t *node = (ztimeout_node_t *) zdlist_item (iterator);
        node->destructor = destructor;
        iterator = zdlist_next (iterator, self->bucket [0]);
    }
}
...
s_dump (ztimeout_t *self)
{
    ...
    for (int index = 0; index < NUM_BUCKETS; ++index) {
        ...
        while (iterator) {
            ...
            iterator = zdlist_next (iterator, NULL);
        }
        printf ("\n");
    }
}
